...
def generate_summary_and_transcript(audio_file_path):
    ...
    model = genai.GenerativeModel("models/gemini-1.5-pro-latest")
    summary_response = model.generate_content(["Please generate the summary of the following audio,The Summary should include in brief the topic of the call, key points discussed, any factual data mentioned, action points for resp stakeholder, and meeting conclusion of the Audio.", audio_file])
    ...
    transcript_args = [(chunk, "Please generate the transcript of this audio segment with speakers name with respect to thier Time frame,display as points.") for chunk in chunk_paths]
    with Pool() as pool:
...
def get_response(chain, context, question, subscription_key):
    ...
    if any(phrase in cleaned_response.lower() for phrase in [
        "The provided text does not contain information", "answer is not available in the context",
